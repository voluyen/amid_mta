...
class Encoder(object):
    ...
    def encode(self, line):
        ...
        if 'instruction' in line.keys():
            ...
            line = {'instruction': line['query'].split('\n')[0],
                    'input': get_input(line['query']),
        ...
        full_tokens = Encoder.tokenizer.encode(prompt + response, add_special_tokens=False) + [
...
def main():
    ...
    with open(os.path.join(args.data_dir, "MetaMathQA-395K.json")) as f:
        raw_data = json.load(f)
    raw_data = random.sample(raw_data, len(raw_data))
    raw_data = raw_data[:args.data_length]
    print(len(raw_data))
    if args.dev_num > 0:
        all_data = {
